...
python3 banned_words_checker.py --solution=deque.hpp --banned-words=banned_words.json
if [[ ! $? -eq 0 ]]
then
...
clang-tidy -extra-arg=-std=c++20 -quiet deque.hpp
if [[ ! $? -eq 0 ]]
then
...
clang-format -i -style='{BasedOnStyle: Google, DerivePointerAlignment: false, PointerAlignment: Left, AlignOperands: true}' deque.hpp &&
git diff --ignore-submodules --color deque.hpp > diff &&
cat diff
if [[ -s diff ]]
then
...
./deque_pt2_stress_test
if [[ ! $? -eq 0 ]]
then
  echo "Стресс тест не пройден"
  exit 1
fi
echo "Stress test achieved"

...
class Deque<T>::common_iterator {
 public:
  ...
  common_iterator(Deque<T>* container, size_t block, size_t position)
...
template <typename T>
Deque<T>::Deque(size_t count, const T& k_value)
    : count_block_((count / kSizeBlock + 1) * 2),
      begin_(this, (count_block_ - 1) / 2, 0),
      end_(this, (count_block_ - 1) / 2, 0),
      buff_(count_block_) {
  ...
  if (size() != other.size()) {
    try {
      ...
      for (size_t i = 0; i < other.count_block_; ++i) {
        // выделяем память не создавая объекты
        buff_.push_back(
            reinterpret_cast<T*>(::operator new(sizeof(T) * kSizeBlock)));
      }
...
template <typename T>
bool Deque<T>::empty() {
  return (end_.position == begin_.position) && (begin_.block == end_.block);
}
